"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require('@angular/core');
var ToduComponent = (function () {
    function ToduComponent() {
        this.todoValue = { name: '', completed: false };
        this.todoItems = [];
    }
    //add todo item to list
    ToduComponent.prototype.addTodoItem = function (newValue) {
        var newConfigObjValue = {
            name: newValue['name'],
            completed: newValue['completed']
        };
        this.todoItems.unshift(newConfigObjValue);
        this.todoValue['name'] = "";
    };
    ToduComponent.prototype.isTodoItemsEmpyt = function () {
        return !!(this.todoItems.length);
    };
    //update the todo completed or not 
    ToduComponent.prototype.updateTodoItemCompleted = function (todoCompletedConfig) {
        var todoUpdatedToggle = todoCompletedConfig['todoCompleted'];
        this.todoItems[todoCompletedConfig['todoIndex']]['completed'] = todoUpdatedToggle;
    };
    ToduComponent.prototype.ngOnInit = function () { };
    ToduComponent = __decorate([
        core_1.Component({
            moduleId: module.id,
            selector: 'todu-component',
            templateUrl: 'todu.component.html',
            styleUrls: ['todu.component.css']
        }), 
        __metadata('design:paramtypes', [])
    ], ToduComponent);
    return ToduComponent;
}());
exports.ToduComponent = ToduComponent;
//# sourceMappingURL=todu.component.js.map